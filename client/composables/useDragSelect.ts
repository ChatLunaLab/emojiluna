...
export function useDragSelect<T>(
    ...
    const selectionBox = reactive<Rect>({
        left: 0,
        top: 0,
        width: 0,
        height: 0
    ...
    const handleMouseDown = (event: MouseEvent) => {
        ...
        if (
            (event.target as HTMLElement).closest(
                'button, .el-checkbox, .action-btn, .prevent-drag-select, .el-upload, .el-input, .el-select'
            )
        ) {
            return
        }
    ...
    const handleMouseMove = (event: MouseEvent) => {
        ...
        items.value.forEach((item) => {
            ...
            const wasSelected = initialSelection.some(
                (i) => i[itemIdKey] === item[itemIdKey]
            )
