        ...
        if (activeTab.value === 'upload') {
            ...
            if (result && result.errors && result.errors.length > 0) {
                const failed = result.errors.map(
                    (it: any) => it.file || it.fileName || it.file || 'unknown'
                )
                ElMessage.warning(
                    t('emojiluna.uploadPartialFailed', {
...
const submitFile = async () => {
    ...
    try {
        ...
        if (duplicates.length > 0) {
            ElMessage.info(
                t('emojiluna.deduplicated', { count: duplicates.length })
            )
        }
